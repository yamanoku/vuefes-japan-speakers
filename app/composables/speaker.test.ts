...
const useFetchSpeaker = async (params?: string) => {
  ...
  const handler = isValidYear(params)
    ? async (year: string) => {
        const { data } = await (globalThis as unknown as { useFetch: typeof useFetch }).useFetch<
          SpeakerInfo[]
        >(`/api/speakers/${year}`);
    ...
    : async (name: string) => {
        ...
        const promises = years.map((year) =>
          (globalThis as unknown as { $fetch: typeof $fetch })
            ...
            .then((speakers) => speakers.map((speaker) => ({ ...speaker, year }))),
        );
        ...
        const filterNameSpeaker = computed(() => {
          ...
          const results = allSpeakers.filter((speaker) =>
            speaker.name.some((speakerName) => speakerName.toLowerCase().includes(searchTerm)),
          );
          return results;
        });
